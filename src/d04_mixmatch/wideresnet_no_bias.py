...
class BasicBlock(nn.Module):
    ...
    def __init__(self, n_features):
        ...
        self.conv1 = nn.Conv2d(n_features, n_features, kernel_size=(3, 3), padding=1, bias=False)

        self.bn2 = nn.BatchNorm2d(n_features)
        self.conv2 = nn.Conv2d(n_features, n_features, kernel_size=(3, 3), padding=1, bias=False)
...
class TransitionBlock(nn.Module):
    ...
    def __init__(self, in_f, out_f, downsample):
        ...
        self.relu = nn.LeakyReLU(negative_slope=0.1)
        self.conv1 = nn.Conv2d(in_f, out_f, kernel_size=(3, 3), padding=1, stride=stride, bias=False)

        self.bn2 = nn.BatchNorm2d(out_f)
        self.conv2 = nn.Conv2d(out_f, out_f, kernel_size=(3, 3), padding=1, bias=False)

        # Shortcut connection for identity to match dimensions
        self.shortcut = nn.Conv2d(in_f, out_f, kernel_size=(1, 1), stride=stride, bias=False)
...
class WideResNetUnbias(nn.Module):

    def __init__(self, depth, k, n_out):
        super(WideResNetUnbias, self).__init__()

        assert (depth - 4) % 6 == 0, "depth must be 6*n + 4"
        n = int((depth - 4) / 6)
        n_features = [16, 16*k, 32*k, 64*k]

        self.conv1 = nn.Conv2d(3, n_features[0], kernel_size=(3, 3), padding=1, bias=False)
...
if __name__ == '__main__':
    ...
    model = WideResNetUnbias(depth=28, k=2, n_out=10)
    model.to(device)
